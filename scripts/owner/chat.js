...
function loadFriendRequests() {
    try {
        const saved = localStorage.getItem('matchfieldOwnerFriendRequests');
        if (saved) {
...
function saveFriendRequests() {
    try {
        localStorage.setItem('matchfieldOwnerFriendRequests', JSON.stringify(friendRequests));
    } catch (error) {
...
function updateFriendButtonStates(userId) {
    // Update "Add To Friend" button in player section
...
// Sample chat data - Owners chatting with players
const chatData = {
    conversations: [
        {
            id: 1,
            ...
            status: "online",
            lastMessage: "Perfect! I'll book it for Saturday at 3 PM",
            ...
            messages: [
                ...
                },
                {
                    id: 5,
                    sender: "Sarah Johnson",
                    text: "Perfect! I'll book it for Saturday at 3 PM",
                    time: "10:40 AM"
                }
            ],
            playerInfo: {
            ...
            messages: [
                {
                    id: 1,
                    sender: "Michael Chen",
                    text: "Hello, is the field available this weekend?",
                    time: "2:30 PM"
                },
                {
                    id: 2,
                    sender: "user",
                    text: "Yes, we have slots available. What day works for you?",
                    time: "3:15 PM"
                },
                {
                    id: 3,
        ...
        },
        {
            ...
            name: "Emma Wilson",
            role: "Player",
            avatar: "https://ui-avatars.com/api/?name=Emma+Wilson&background=8B5CF6&color=fff&size=128",
            status: "online",
            ...
            messages: [
                {
                    id: 1,
                    sender: "user",
                    text: "Your booking for tomorrow is confirmed",
                    time: "4:20 PM"
                },
                {
                    id: 2,
        ...
        },
        {
            ...
            name: "David Martinez",
            role: "Player",
            avatar: "https://ui-avatars.com/api/?name=David+Martinez&background=007BFF&color=fff&size=128",
            status: "online",
            lastMessage: "I'm interested in a group booking",
            ...
            messages: [
                {
                    ...
                    text: "I'm interested in a group booking",
                    time: "11:00 AM"
                }
            ],
            playerInfo: {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Hide player info sidebar initially
    const playerInfoSidebar = document.getElementById('playerInfoSidebar');
    if (playerInfoSidebar) {
        playerInfoSidebar.style.display = 'none';
    }
...
function selectConversation(chatId) {
    ...
    // Update player info sidebar (but don't show it yet - user clicks profile to show)
    updatePlayerInfo(conversation);
    // Hide sidebar initially - will show when user clicks on profile
    const playerInfoSidebar = document.getElementById('playerInfoSidebar');
    if (playerInfoSidebar && conversation.playerInfo) {
        playerInfoSidebar.style.display = 'none';
    }
...
// Update player info sidebar (updates data but doesn't show it - user clicks profile to show)
function updatePlayerInfo(conversation) {
    if (!conversation.playerInfo) {
        // Hide player info if no info available
        const playerInfoSidebar = document.getElementById('playerInfoSidebar');
        if (playerInfoSidebar) {
            playerInfoSidebar.style.display = 'none';
        }
    ...
    const playerInfo = conversation.playerInfo;
    ...
    if (playerName) {
        playerName.textContent = conversation.name;
    }
    
    if (playerAvatar) {
        playerAvatar.src = conversation.avatar;
    }
    
    if (playerStatus) {
        playerStatus.textContent = conversation.status === 'online' ? 'Online' : 'Offline';
        playerStatus.style.color = conversation.status === 'online' ? '#10B981' : '#6B7280';
    }
    ...
    if (totalBookings) totalBookings.textContent = playerInfo.totalBookings;
    if (memberSince) memberSince.textContent = playerInfo.memberSince;
    if (playerRating) playerRating.textContent = playerInfo.rating;
...
function simulateResponse(conversation) {
    ...
    const responses = [
        ...
        "That sounds great!",
        "Perfect, I'll confirm the booking."
    ];
...
function setupEventListeners() {
    ...
    // Add to friend button (in player section)
    const addToFriendBtn = document.getElementById('addToFriendBtn');
    if (addToFriendBtn) {
    ...
    if (viewPlayerProfileBtn) {
        viewPlayerProfileBtn.addEventListener('click', () => {
            if (chatData.currentChatId) {
                const conversation = chatData.conversations.find(c => c.id === chatData.currentChatId);
                if (conversation) {
                    showNotification('Viewing player profile for ' + conversation.name, 'info');
                    // Add navigation logic here if you have a player profile page
                } else {
                    showNotification('Please select a conversation first', 'info');
                }
            } else {
    ...
    // Chat header click to toggle player info sidebar
    const chatHeader = document.getElementById('chatHeader');
    if (chatHeader) {
        chatHeader.addEventListener('click', () => {
            togglePlayerInfoSidebar();
        });
...
// Toggle player info sidebar visibility
function togglePlayerInfoSidebar() {
    const playerInfoSidebar = document.getElementById('playerInfoSidebar');
    if (!playerInfoSidebar) return;
    
    const conversation = chatData.conversations.find(c => c.id === chatData.currentChatId);
    if (!conversation || !conversation.playerInfo) {
        // Don't show sidebar if no player info available
        playerInfoSidebar.style.display = 'none';
        return;
    }
    
    // Toggle visibility
    const isVisible = playerInfoSidebar.style.display === 'flex';
    playerInfoSidebar.style.display = isVisible ? 'none' : 'flex';
}
...
function saveChatToStorage() {
    try {
        localStorage.setItem('matchfieldOwnerChatData', JSON.stringify(chatData));
    } catch (error) {
...
function loadChatFromStorage() {
    try {
        const saved = localStorage.getItem('matchfieldOwnerChatData');
        if (saved) {
...
const defaultUsersList = [
    { id: 1, name: "Sarah Johnson", role: "Player", avatar: "https://ui-avatars.com/api/?name=Sarah+Johnson&background=EF4444&color=fff&size=128", status: "online" },
    { id: 2, name: "Michael Chen", role: "Player", avatar: "https://ui-avatars.com/api/?name=Michael+Chen&background=10B981&color=fff&size=128", status: "offline" },
    { id: 3, name: "Emma Wilson", role: "Player", avatar: "https://ui-avatars.com/api/?name=Emma+Wilson&background=8B5CF6&color=fff&size=128", status: "online" },
    { id: 4, name: "David Martinez", role: "Player", avatar: "https://ui-avatars.com/api/?name=David+Martinez&background=007BFF&color=fff&size=128", status: "online" }
];
    ...
    modalOverlay.innerHTML = `
        <div class="friend-modal">
            ...
            <div class="friend-modal-content">
                <p>Search for a player by name or ID to send a friend request:</p>
