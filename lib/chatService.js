...
async function canStartDirectConversation(meId, other) {
  ...
  // Player ↔ Owner (venue communication)
  const roles = new Set([me.role, other.role]);
  if (roles.has('PLAYER') && roles.has('OWNER')) {
    return { ok: true };
  }
  // Shared booking relationship
  ...
  if (sharedBooking) return { ok: true };

  return {
    ok: false,
    status: 403,
    error: 'You can only message field owners, players from your bookings, or support/admin.'
  };
}
...
async function getOrCreateDirectConversation(meId, otherId) {
  ...
  const allowed = await canStartDirectConversation(meId, other);
  if (!allowed.ok) {
    return allowed;
  }
