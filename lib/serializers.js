...
const { resolvePublicUrl, isSupabaseRef } = require('./storage');

function resolveUserAvatar(user) {
  const raw = (user && (user.avatarUrl || user.avatar)) || null;
  if (!raw || typeof raw !== 'string') return null;
  if (raw.startsWith('data:image/')) return raw;
  if (raw.includes('storage/private/') || raw.startsWith('sb://matchfield-private/')) return null;
  if (isSupabaseRef(raw)) return resolvePublicUrl(raw);
...
function imageUrlsFromField(field) {
  if (!field) return [];
  const { resolvePublicUrl } = require('./storage');
  if (Array.isArray(field.images) && field.images.length) {
    if (typeof field.images[0] === 'string') {
      return field.images.map((url) => resolvePublicUrl(url) || url).filter(Boolean);
    }
    return field.images
      .slice()
      .sort((a, b) => (a.displayOrder || 0) - (b.displayOrder || 0))
      .map((img) => resolvePublicUrl(img.storagePath, img.publicUrl))
      .filter(Boolean);
  }
  return [];
}
...
/** Authenticated download path only — never embed JWTs in query strings. */
function documentApiUrl(fieldId, type) {
  if (!fieldId || !type) return null;
  return `/api/fields/${fieldId}/documents/${encodeURIComponent(type)}`;
}

function documentUrlsForAuthorizedViewer(field) {
  ...
  docs.forEach((doc) => {
    ...
    // Never emit raw data URLs, private filesystem paths, or access tokens
    const url = documentApiUrl(field.id, doc.type);
  ...
  out.reviews = field.reviews.map((r) => ({
    ...
    user: r.user
      ? {
          ...
          avatar: resolveUserAvatar(r.user)
        }
      : null
  }));
}
...
function publicFieldSerializer(field) {
  ...
  if (field.owner) {
    out.owner = {
      ...
      avatar: resolveUserAvatar(field.owner)
...
function ownerFieldSerializer(field, _opts = {}) {
  if (!field) return null;
  const out = baseFieldPayload(field);
  out.phone = field.phone || null;
  ...
  Object.assign(out, documentUrlsForAuthorizedViewer(field));
  if (field.owner) {
    out.owner = {
      id: field.owner.id,
      fullName: field.owner.fullName,
...
function adminFieldSerializer(field, _opts = {}) {
  if (!field) return null;
  const out = ownerFieldSerializer(field);
  if (field.owner) {
    out.owner = {
      id: field.owner.id,
      fullName: field.owner.fullName,
 ...
 */
function serializeField(field, { includeOwner = true, variant } = {}) {
  if (!field) return null;
  if (variant === 'admin') return adminFieldSerializer(field);
  if (variant === 'owner') return ownerFieldSerializer(field);
  ...
  return {
    ...
    organizer: booking.organizer
      ? {
          ...
          avatar: resolveUserAvatar(booking.organizer),
          ...
          return {
            ...
            user: p.user
              ? {
                  ...
                  avatar: resolveUserAvatar(p.user),
                  email: p.user.email
                }
              : null
          };
